...
import { htmlElements, svgElements, getTag, isDynamicCss, dynamicTest, isFont, trimAttrWhitespace, cumbersomeTrim } from './utils';
import { processWithPostHtml, parseAttrsToLocals, parseTemplate } from './parser';
import { postcssScopedParser } from './css';
let postcssrc_sync;
let processor;
...
export function parse(options) {
    function parseHandle(options, isPage) {
        return async function (tree) {
            ...
            tree.walk((node) => {
                ...
                if (component) {
                    ...
                    promises.push(readComponentFile(component, options.encoding)
...
async function collectCssAndJs(tree, options) {
    ...
    list.forEach((resolveId) => {
        ...
        tree.match(match(css.to), (node) => {
            ...
            switch (node.tag) {
                case 'head':
                    const styled = node.content && node.content.find(item => {
...
function parseStyleAndScript(componentNode, component, options) {
    getPostcssConfigSync();
    const { stylePreprocessor, styled: optionStyled, addClassIncludes, assets, noflip, cssjanus } = options;
    ...
    return async function (tree) {
        ...
        tree.walk((node) => {
            ...
            if (scopedCls && scopedHash && node.tag) {
                ...
                if ((scopedCls.tags[node.tag] || includes(attrs)) &&
 ...
 */
function matchComponents(tree, { from: htmlFilePath, prefix, join, slash, }) {
    const components = {};
    // <view:components>
