...
async function collectCssAndJs(tree, options) {
  ...
  const jsCache = /* @__PURE__ */ new Map();
  const jsExtract = (obj) => {
  ...
  const invalidSelector = /* @__PURE__ */ new Set();
...
function htmlConversion(html) {
  ...
  return html.replace(syntaxReg$1, _matchfn).replace(syntaxReg2, _matchfn);
}
