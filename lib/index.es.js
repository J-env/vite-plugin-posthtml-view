...
function parseAttrsToLocals(locals, attrs, options) {
  const $attrs = {};
    ...
    tree.match(match(css.to), (node) => {
      ...
      switch (node.tag) {
        case "head":
          ...
          if (styled2) {
            styled2.content = [...styled2.content, css.css].filter(Boolean);
          } else {
            const element2 = {
              tag: "style",
            ...
            if (node.content) {
              node.content = [...node.content, element2];
            } else {
...
function parseComponent(node, component, options) {
  ...
  return async function(tree) {
    ...
    tree.walk((node) => {
      ...
      if (scopedClasses && scopedHash && node.tag) {
        ...
        if ((scopedClasses.tags[node.tag] || scopedClasses.classNames.some((c) => classNames.includes(c))) && !classNames.includes(scopedHash)) {
      ...
      assets.attributes.forEach((attrKey) => {
        if (node.attrs[attrKey]) {
          ...
          if (!String(rawUrl).trim() || isExternalUrl(rawUrl) || isDataUrl(rawUrl) || rawUrl.startsWith("#") || dynamicTest(rawUrl)) {
            return;
          }
...
async function requireMock(tspath, hot = true) {
  let mock = null;
  if (tspath) {
...
async function phpRenderToHtml(filename, input) {
    ...
    return (await posthtml([]).use((tree) => {
      tree.match(match("head"), (head) => {
        ...
        tree.match(match("style[__posthtml_view_css__]"), (style) => {
          const content2 = [].concat(style.content || "").join("").trim();
          if (content2 && !css.includes(content2)) {
            css.push(content2);
          }
          style.tag = false;
          delete style.content;
      ...
      for (const bundle of Object.values(bundles)) {
        ...
        if (bundle.type === "asset" && filter(bundle.fileName)) {
          ...
          if (rtl && type === "new-html") {
            const html = (await posthtml([]).use((tree) => {
              tree.match(match("html"), (node) => {
                node.attrs = node.attrs || {};
                node.attrs.dir = "rtl";
                return node;
...
function getTargetNodeIndex(content) {
  ...
  for (let i = length - 1; i >= 0; i--) {
    ...
    const hasModule = hasLinkCss || elem.tag === "link" && attrs.rel === "modulepreload" || elem.tag === "script" && attrs.type === "module" && attrs.src;
    if (hasModule) {
      index = i;
      break;
    }
